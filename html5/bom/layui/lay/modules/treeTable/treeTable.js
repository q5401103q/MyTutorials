...
layui.define(['layer', 'laytpl', 'form'], function (exports) {
    ...
    TreeTable.prototype.init = function () {
        ...
        $tbBox.css('min-width', tbWidth.minWidth + 1); //总特么有滚动条，差一像素，给手动加1
        $headTb.parent().css('min-width', tbWidth.minWidth + 1);//总特么有滚动条，差一像素，给手动加1
        if (tbWidth.setWidth) {
    ...
    TreeTable.prototype.bindEvents = function () {
        ...
        // 绑定行双击事件 - 改为右键事件
        $tbody.off('contextmenu.mouseRightMenu').on('contextmenu.mouseRightMenu', 'tr', function () {
            layui.event.call(this, MOD_NAME, 'mouseRightMenu(' + tbFilter + ')', commonMember.call(this, {}));
        });
    ...
    TreeTable.prototype.renderBodyTd = function (d, indentSize, index, $td) {
        ...
        if (index == treeOption.iconIndex) {
            ...
            if (options.tree.onlyIconControl) {
                tdStr += '</span>';
                tdStr += ('<span class=' + d.icon + '>' + fieldStr + '</span>');
            } else {
                tdStr += ('<span class=' + d.icon + '>' + fieldStr + '</span>');
                tdStr += '</span>';
            }
    ...
    function getThead(options) {
        var htmlStr = '<tr class="tree-header">';
        for (var i = 0; i < options.cols.length; i++) {
